...
module.exports = {
	...
	getTagWithPseudo: function(viewcode,operator,selector){
		...
		if(tagname.match(/:[a-zA-Z-]+/) == ':nth-child'){
			var nthvalue = tagname.match(/\((.*)\)/);
		...
		if(tagname.match(/:[a-zA-Z-]+/) == ':first-child'){
			var nthchild = 1;
			tagname = tagname.replace(/:first-child/g,"");
			selector.tag = tagname;
		...
		if(tagname.match(/:[a-zA-Z-]+/) == ':last-child'){
			var nthchild = 1;
			tagname = tagname.replace(/:last-child/g,"");
			selector.tag = tagname;
		...
		if(tagname.match(/:[a-zA-Z-]+/) == ':not'){
			var notvalue = tagname.match(/\((.*)\)/);
			notvalue = notvalue[1];
	...
	getTagSelector: function(selector){
		...
		if(selector.match(/\s*\:\s*first-child|\s*\:\s*nth-child\s*|\s*\:\s*last-child|\:not|\[(.*?)(?=\])/g)){
			if(selector.match(/:[a-zA-Z-]+/) == ':nth-child'){
				targetnthchild = selector.match(/\s*\:\s*nth-child/g);
				targetnumber = selector.match(/\(+.*?\)+/g);
				tagselector = targetnthchild + targetnumber;
			}
			if(selector.match(/:[a-zA-Z-]+/) == ':first-child'){
				tagselector = selector.match(/\s*\:\s*first-child/g);
			}
			if(selector.match(/:[a-zA-Z-]+/) == ':last-child'){
				tagselector = selector.match(/\s*\:\s*last-child/g);
			}
			if(selector.match(/:[a-zA-Z-]+/) == ':not'){
				targetnotchild = selector.match(/\:not/);
				tragetnotvalue = selector.match(/\(+.*?\)+/g);
				tagselector = targetnotchild + tragetnotvalue;
			}
			if(selector.match(/\[(.*?)(?=\])/g)){
