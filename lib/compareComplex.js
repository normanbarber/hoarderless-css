...
module.exports = {
		...
		for(var i in viewcode){
			for(var j in viewcode[i].dom){
				if(!tag2.selectors && viewcode[i].dom[j].tag === tag1.tag){
					...
					matchfound = findmatch.getTagWithAttributes(viewcode[i].dom[j].attributes,tag2.selectors);
	...
	compare_negation: function(viewcode, attributes, selector, notvalue){
		...
		var sel = findmatch.getTagWithPseudo(viewcode,null,tag);
		var onetag = findmatch.getTag(viewcode,sel,null);
