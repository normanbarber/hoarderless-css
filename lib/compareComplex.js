...
module.exports = {
	...
	compare_DirectChildren: function(viewcode, operator, tag2, tag1, tagname2){
        ...
        // WORKS div#container > p > p{ }  should comment and does not
        var filterdirectchild;
        _.each(viewcode, function(code){
            filterdirectchild = [];
            var dom = code.dom;
            filterdirectchild = dom.filter(function (tag) {
...
//                if(!tag2.selectors && tag.tag === tag1.tag){
//                    return tagchildren.filter(function (childname) { return childname === tagname2});
//                }
...
//		return false;
	...
	compare_nthChild: function(viewcode, selector, tagname, n){
        ...
        _.each(viewcode, function(code){
            var dom = code.dom;
            filterchild = dom.filter(function (tag) {
                var siblingscount = 1;
                var tagsiblings = tag.siblings;
                var matchfound = findmatch.getTagWithAttributes(tag.attributes,selector.selectors);

                filtersiblings = tagsiblings.filter(function (sib) {
                    if(sib === tagname){
                        siblingscount++;
                        return siblingscount >= parseInt(n);
                    }
                });

                // if tag passed in equals the tag being looped and n = 1 then return
                // or if match is found
                return tag.tag === tagname && parseInt(n) === 1 || matchfound || filtersiblings.length > 0;

            })
        })

        if(filterchild.length > 0 || filtersiblings.length > 0)
            return true;
...
//		return false;
	...
	compare_Adjacent: function(viewcode, tagname1, tagname2){
        ...
        var filteradjacent;
        for(var i in viewcode){
            var dom = viewcode[i].dom;
            filteradjacent = dom.filter(function (t) {
                return t.tag === tagname1 && t.tabindents === dom[0].tabindents && dom[0].siblings[0] === tag
            });
        }
//        console.log('filteradjacent');
//        console.log(filteradjacent);
