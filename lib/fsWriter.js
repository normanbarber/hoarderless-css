...
module.exports = {
	services: {
		/*  called from codeWriterMain.js
			loops css code compares with the
			unused selectors object. when it finds
			a match it comments the css rule
			eventually writing cleaned code
			to a new css file */
							...
							for(var type in selectors.unused){
								...
								for(var v in unused){
									for(var x in unused[v].selector){
                                       ...
                                       // cssselector = cssselector.replace(/\[/g, '[').replace(/\]/g, ']');
										if(cssstr.slice(0,1) == ' ')
