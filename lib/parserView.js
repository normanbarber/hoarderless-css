	...
	start: function(filename,codearray){
		...
		for(var k in codearray) {
			...
			if(codearray[k].match(/ng-style/g)){
				codearray[k] = codearray[k].replace(/([a-z]+-)?style="(.*?)",?|ng\s*:\s*style="(.*?)",?/g,"");
			}

			// todo: handle more ng-class using [class1, class2, class3]
			var ngclassmatch = codearray[k].replace(/<~>/g,"").match(/([a-z]+-)?ng-class([-a-z]+)?="+\{?(.*)\}?"+,?|([a-z]+:)?ng:class([:a-z]+)?="+\{?(.*)\}?"+,?/g);
			...
			if(ngclassmatch){
				for(var ngindex in ngclassmatch){
					...
					var finallyclasses = ngclassreplace.match(/"+(.*?)"+/g);
					for(var matchindex in finallyclasses){
						var matchclass = finallyclasses[matchindex].replace(/"/g,"");
						ngclassarray.push(matchclass);
					}
				}
			}
			if(ngclassarray){
				for(var i in ngclassarray){
					classesindex = allclasses.length;
					allclasses[classesindex] = {"filename" : filename, "selector" : ngclassarray[i]};
					classarray.push(ngclassarray[i]);
				}
			}
			// remove ng-class from this line of code. combined with class attribute
			codearray[k] = codearray[k].replace(/([a-z]+-)?ng-class([-a-z]+)?="\{?(.*)\}?"+,?|([a-z]+:)?ng:class([:a-z]+)?="+\{?(.*)\}?"+,?/g,"");
