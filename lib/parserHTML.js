...
module.exports = {
	start: function(filename,code,viewtag){
		...
		_.each(tagsarray, function(tag, key){
			...
			if(code[0].match(/input\b|link\b|base\b|img\b|meta\b|area\b|br\b|col\b|command\b|embed\b|hr\b|keygen\b|param\b|source\b|track\b|wbr\b/) && tagmatch[0].substr(-1) != '/')
				tagmatch[0] = code[0] + '/';

			if(code[0].substr(0,1) != '/'){
				treenode++;
				if(code[0] === 'script' || isScript || (code[0] == 'script' && tagmatch[0].substr(-1) === '/')){
					isScript = true;
					linenumber--;
				...
				if(code[0] === '/script' || code[0] === 'script' || isScript)
					linenumber--;
		...
		/*  push all classes ids and attriubtes to their respective property  */
	...
	getChildren: function(tagsarray,tag,attributes,linenumber,treenode){
		...
		_.each(tagsarray, function(looptag){
			...
			loopline++;
			if(isScript || innertagname[0] === 'script'){
				isScript = true;
				loopline--;
			}

			if(innertagname[0].match(/input\b|link\b|base\b|img\b|meta\b|area\b|br\b|col\b|command\b|embed\b|hr\b|keygen\b|param\b|source\b|track\b|wbr\b/) && innertag[0].substr(-1) != '/')
				innertag[0] = innertagname[0] + '/';
