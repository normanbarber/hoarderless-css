...
module.exports = {
	start: function(filename,code,viewtag){
		...
		_.each(tagsarray, function(tag, key){
			...
			if(code[0].substr(0,1) != '/'){
				treenode++;
				if(code[0] === 'script' || isScript === true){
					isScript = true;
					linenumber--;
	...
	getChildren: function(tagsarray,tag,attributes,linenumber,treenode){
		...
		_.each(tagsarray, function(looptag){
			...
			if(innertagname[0] === 'script' || isScript === true || innertagname[0] === '/script'){
				loopline--;
			}
			...
			*/
			...
			if(innertagname[0] === 'script' && innertagname[0].substr(0,1) != '/')
				isScript = true;
			...
			*/
			...
			if(isScript === true || innertagname[0].substr(0,1) === '/' || innertag[0].substr(-1) === '/'){
