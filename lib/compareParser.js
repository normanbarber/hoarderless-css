...
module.exports = {
	...
	cssParser: function(viewdom,selector){
		...
		tag = allattributes ? this.getViewElement(viewdom,allattributes,tag).tag : this.identifyTagName(selector);
		tagelement = allattributes ? this.getViewElement(viewdom,allattributes,tag) : this.findViewTagMatch(tag, viewdom);
		tagelement.tag  = pseudo ? pseudo : tagelement.tag;
