...
module.exports = {
	...
	cssParser: function(viewdom,selector){
		...
		if(selector && selector.match(/:[a-zA-Z-]+/) == ':nth-child' || selector.match(/:[a-zA-Z-]+/) == ':first-child' || selector.match(/:[a-zA-Z-]+/) == ':last-child' || selector.match(/:[a-zA-Z-]+/) == ':not'){
			...
			selector = selector.replace(/\(.*?\)+/g,"").replace(/:nth-child/g,"").replace(/:first-child/g,"").replace(/:last-child/g,"").replace(/:not/g,"");
		}
		...
		classarray = selector.match(/\.[a-zA-Z-0-9]+[^.#\[]?/g);
		idsarray = selector.match(/\#[a-zA-Z-0-9]+[^.#\[]?/g);
		...
		selector = attributes && attributes[0] ?  attributes[0] : null;

		if(selector && !this.findViewTagMatch(selector,viewdom))
			return false;

		attributesformat = !attributesformat ?  this.formatAttributes(classarray,idsarray) : attributesformat;
		tag.tag = selector;
		tag.attributes = attributesformat;
		return tag;
