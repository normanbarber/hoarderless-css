...
var compareCode = {
    ...
    findUnusedSelectors: function(viewcode,selectorsplit){
        // recursive strategy loop thru all selectors 2 at a time
        ...
        if(viewcode){
            // checking if the second selector is found
            ...
            // find any matches to nth-child first-child last-child :not or attriute selector
    ...
    matchTags: function(viewfiles,tag,operator){
        // compares tag and its selectors with the view searching for a match
        ...
        // else its just a tag with no classes or ids
        var view_code_obj = [];
            ...
            for(var j in viewcode){
                ...
                if(tag.selectors){
                    ...
                    if(matchfound || childarray.indexOf(viewcode[j].tag) >= 0 || siblingsarray.indexOf(viewcode[j].tag) >= 0){
                        ...
                        // child_array_index is the number of children we are adding to the new dom. When the index reaches childarray.length it will stop
                        // may need to do one more check for tabindents so it doesnt save any parent elements to childarray or siblingsarray. dont need parent tags right now
                        ...
                        if(siblingsarray.indexOf(viewcode[j].tag) >= 0 && siblings_array_index <= siblingsarray.length && (operator === '~' || operator === '+')){
                            // only want save/push this element if its a child or sibling that why it checks that the tabindents is greater than(child) or equal to(sibling) the curren tag
                            if(viewcode[j].tabindents > tag.tabindents){
